 ...
 * @author hp
 */
public class Ring_Route_bus_problem {
    
    public static float totalINR_Busruning(String Source, String Destination) {
        ...
        for (int i = 0; i < busstop.length; i++) {
            if (busstop[i].equalsIgnoreCase(Source)) {
            
                sour = i;
              //  System.out.println(sour);
            }
            if (busstop[i].equalsIgnoreCase(Destination)) {
                Dest = i;
    ...
    public static void main(String args[]) {
        System.out.println("enter soure bus Stop");
        String source = new Scanner(System.in).next();
        String destination = new Scanner(System.in).next();
        System.out.println(totalINR_Busruning(source, destination));
